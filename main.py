...
if st.session_state.get("ai_result"):
    ...
    if ai_data:
        ...
        with ce:
            ...
            _pass = ai_data["hard_limit_pass"]
            if _hfc == 0:
                hard_note = "ทุกตัวผ่าน"
            elif _pass:
                hard_note = f"ตัวที่เลือกผ่าน ({_hfc} ตัวอื่นถูกตัด)"
            else:
                hard_note = f"ตัวที่เลือกไม่ผ่าน (กำลัง DC เกิน Max PV Power)"
            st.markdown(
